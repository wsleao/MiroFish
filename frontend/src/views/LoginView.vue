  ...
  <main class="login-page">
    ...
    <section class="login-card">
      <div class="card-header">
        ...
        <h2>Entrar no NSC</h2>
        <p>Use seu login para acessar simulações, histórico e relatórios.</p>
      </div>

      <form @submit.prevent="handleLogin" class="login-form">
        <label>
          E-mail
          <input v-model="email" type="email" autocomplete="email" placeholder="admin@nsc.local" required />
        </label>
        <label>
          Senha
        ...
        <button :disabled="loading" type="submit">
          <span>{{ loading ? 'Validando acesso...' : 'Entrar' }}</span>
          <strong>→</strong>
        </button>
      </form>

      <div class="login-note">
        <strong>MVP SaaS:</strong> autenticação inicial para proteger a experiência. A próxima etapa é autenticação persistente por tenant, usuários e permissões no banco.
      </div>
    </section>
  </main>
</template>
...
.hero-panel {
  ...
  background:
    radial-gradient(circle at 15% 20%, rgba(83, 109, 254, .32), transparent 28%),
    radial-gradient(circle at 75% 80%, rgba(0, 209, 178, .24), transparent 30%),
...
.brand-mark {
  ...
  height: 76px;
  border-radius: 24px;
  background: linear-gradient(135deg, #7c9cff, #00d1b2);
  color: #061020;
  display: grid;
  place-items: center;
  font-weight: 900;
  font-size: 24px;
  box-shadow: 0 20px 70px rgba(0, 209, 178, .25);
  margin-bottom: 32px;
}
...
@media (max-width: 900px) {
  ...
  .hero-panel { padding: 42px 24px; min-height: 42vh; }
  .login-card { margin: 24px auto 48px; }
}
</style>
